...
RESOURCE = os.environ.get(
    ...
    "projects/103761399075/locations/us-east1/reasoningEngines/8651889873201922048",
)
    ...
    async with httpx.AsyncClient(headers=_auth_headers(), timeout=120) as client:
        ...
        async for event in a2a_client.send_message(SendMessageRequest(message=msg)):
            if isinstance(event, tuple):
                ...
                # Handle StreamResponse from a2a-sdk 1.1+
                if hasattr(event, "HasField"):
                    if event.HasField("task") and event.task.context_id:
                        _contexts[user_id] = event.task.context_id
                    elif event.HasField("artifact_update") and event.artifact_update.context_id:
                        _contexts[user_id] = event.artifact_update.context_id
                    elif event.HasField("status_update") and event.status_update.context_id:
